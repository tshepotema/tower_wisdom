...
public class ScoresContentProvider extends ContentProvider {
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		// Set the table
		queryBuilder.setTables(ScoresTable.TABLE_SCORES);
		...
		case SCORE_ID:
			...
			queryBuilder.appendWhere(ScoresTable.COLUMN_ID + "="
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch (uriType) {
		case SCORES:
			id = sqlDB.insert(ScoresTable.TABLE_SCORES, null, values);
			break;
		default:
			throw new IllegalArgumentException("Unknown URI: " + uri);
		}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (uriType) {
		case SCORES:
			rowsDeleted = sqlDB.delete(ScoresTable.TABLE_SCORES, selection,
					selectionArgs);
			break;
		case SCORE_ID:
			String id = uri.getLastPathSegment();
			if (TextUtils.isEmpty(selection)) {
				rowsDeleted = sqlDB.delete(ScoresTable.TABLE_SCORES,
						ScoresTable.COLUMN_ID + "=" + id, null);
			} else {
				rowsDeleted = sqlDB.delete(ScoresTable.TABLE_SCORES,
						ScoresTable.COLUMN_ID + "=" + id + " and " + selection,
						selectionArgs);
			}
			break;
		default:
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		switch (uriType) {
		case SCORES:
			rowsUpdated = sqlDB.update(ScoresTable.TABLE_SCORES, values, selection,
					selectionArgs);
			break;
		case SCORE_ID:
			String id = uri.getLastPathSegment();
			if (TextUtils.isEmpty(selection)) {
				rowsUpdated = sqlDB.update(ScoresTable.TABLE_SCORES, values,
						ScoresTable.COLUMN_ID + "=" + id, null);
			} else {
				rowsUpdated = sqlDB.update(ScoresTable.TABLE_SCORES, values,
						ScoresTable.COLUMN_ID + "=" + id + " and " + selection,
	...
	private void checkColumns(String[] projection) {
		String[] available = { ScoresTable.COLUMN_SCORE, ScoresTable.COLUMN_GROUPID, ScoresTable.COLUMN_ID };
		if (projection != null) {
